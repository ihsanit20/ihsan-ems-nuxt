...
import type { Gender, ResidentialType, GuardianType } from "~/types";

type SelectItem = { label: string; value: any };

useHead({ title: "Add New Student" });
...
onMounted(() => {
  ...
  addressStore.fetchDivisions().catch(() => {});
});
...
async function handleSubmit() {
  ...
  try {
    ...
    await studentStore.create(formData as any);
...
<template>
  <div class="space-y-6">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <UCard>
        <template #header>
          <h3 class="text-lg font-semibold">Guardian Information (Optional)</h3>
        </template>
      ...
      <div class="flex justify-end gap-3">
        ...
        <UButton type="submit" :loading="loading">Add Student</UButton>
      </div>
    </form>
  </div>
</template>
